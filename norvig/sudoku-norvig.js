...
each(squares, function (s) {
	units[s] = filter(unitlist, function (u) {
		return contains(u, s);
	});
});
...
each(squares, function (s) {
	var result = [];
	peers[s] = set(
		filter(Array.prototype.concat.apply([], units[s]), function (s2) {
			return s2 != s;
		}),
	);
});
...
function test() {
	...
	assert(
		all(map(squares), function (s) {
			return units[s].length == 3;
		}),
	);
	assert(
		all(map(squares), function (s) {
			return peers[s].length == 20;
		}),
	);

	var sunits = map(units['C2'], function (u) {
		return u.toString();
	});
	assert(
...
function search(values) {
	...
	var s = candidates[0];
	return some(chars(values[s]), function (d) {
		return search(assign(copy(values), s, d));
	});
}
...
function fromFile(filename, sep) {
	...
	return filter(txt.replace(/^\s*|\s*$/g, '').split(sep), function (l) {
		return l.length > 0;
	});
}
...
function solveAll(grids, name, showif) {
	...
	var N = grids.length;
	var seconds = function (nr) {
		return (nr / 1000.0).toFixed(2);
	};
	if (N > 1) {
		print(
			'Solved ' +
...
function solved(values) {
	...
	function unitsolved(unit) {
		var udigits = map(unit, function (s) {
			return values[s];
		});
		udigits.sort();
	...
	return (
		values !== false &&
		all(unitlist, function (unit) {
			return unitsolved(unit);
		})
	);
}
